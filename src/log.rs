...
pub fn read_last_lines_as_byte_deque(file: &mut File, lines: usize) -> Result<VecDeque<u8>, Error> {
    // The integer types were chosen to avoid overflows when converting between signed and unsigned
    // integers. u16 is used here so that we can support 16bit platforms with simple integer casts
    // (FIXME Not sure if pueue even is supported on any 16bit platforms)
    ...
    'outer: loop {
        ...
        // Safe to cast chunk_size from u16 to u64
        let next_chunk_size = if position > chunk_size as u64 {
            chunk_size
        } else {
            // Safe to cast position to u16 since it is <= chunk_size which is u16
            position as u16
        };
        // Safe to cast next_chunk_size from u16 to i64
        ...
        // Safe to cast next_chunk_size from u16 to usize
